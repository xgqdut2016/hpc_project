...
#pragma omp declare simd //申明向量化函数
...
float Jacobi(float *f_1d, float *u_old, float *u_new, float dx, float dy){
	...
	#pragma omp for simd schedule(runtime) private(i,j) collapse(2)
    ...
    #pragma omp barrier
    #pragma omp for simd private(i,j,resid) schedule(runtime) collapse(2)
...
//solve
    ...
    while (k < max_iter) {
        ...
        #pragma omp parallel 
        {  
            #pragma omp for simd reduction(max:L1_err) private(tid) schedule(runtime) private(i,j) collapse(2)
            for(i = 0; i < M; i++){
                for(j = 0; j < N; j++){
            ...
            #pragma omp single
            {
                ...
                if (k%1000 == 0){
	                printf("print at tid = %d,%d iteration,error: %3e,L1_err:%.3e\n",tid,k,error,L1_err);
	            }
            }
        }  
