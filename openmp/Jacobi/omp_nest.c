...
//solve
    ...
    while (k < max_iter) {
	    ...
	    #pragma omp parallel default(shared) 
        {
            ...
            for(i = 0; i < M; i++){
                #pragma omp parallel for num_threads(4) default(shared) private(j) 
	            for (j = 0;j < N;j++){
		            u_old[i*N + j] = u_new[i*N + j];
		        }
	        }
            #pragma omp barrier
            #pragma omp for private(i,j,tid,resid,sub_time0,sub_time1)  \
            reduction(+:error)  
            for(i = 0; i < M; i++){
                sub_time0 = get_walltime();
                #pragma omp parallel for num_threads(4) default(shared) private(j,resid) reduction(+:error)
		        for (j = 0; j < N; j++){
                    if (i == 0 || i == M - 1 || j == 0 || j == N - 1){
                        continue;
                    }
                ...
                sub_time1 = get_walltime() - sub_time0;
        ...
        //打印不同线程的工作
        #pragma omp parallel for reduction(max:L1_err) private(tid) 
