...
//solve
    ...
    while (k < max_iter) {
        ...
        #pragma omp parallel for default(shared) private(i,j,tid,resid) schedule(static,chunk) \
        reduction(+:error)  
	    //{注意使用omp parallel for不需要花符号
        ...
        //打印不同线程的工作
        #pragma omp parallel for reduction(max:L1_err) private(tid) schedule(static,chunk) ordered
            ...
            for(i = 0; i < M; i++){
                ...
                if (k == 1){
                    ...
                    #pragma omp ordered
                    {
                        printf("chunk = %d,nthreads = %d,tid = %d,i = %d\n",chunk,nthreads,tid,i);
                    }
