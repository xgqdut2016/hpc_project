...
float Jacobi(float *f_1d, float *u_old, float *u_new, float dx, float dy){
    ...
    float resid = 0;
    float error = 0;
	#pragma omp for private(i,j) schedule(runtime) //collapse(2)
    ...
    #pragma omp barrier
    #pragma omp for private(i,j,resid) schedule(runtime) //collapse(2)
...
//solve
    ...
    while (k < max_iter) {
        ...
        #pragma omp parallel 
        {  
            #pragma omp for reduction(max:L1_err) private(tid) schedule(runtime) //collapse(2)
            for(i = 0; i < M; i++){
                for(j = 0; j < N; j++){
