...
float Jacobi(float *f_1d, float *u_old, float *u_new, float dx, float dy, float error,int sub_nthreads){
    int i,j;
	...
	#pragma omp for private(i,j) schedule(runtime)
       	...
       	for(i = 0; i < M; i++){
            #pragma omp parallel num_threads(sub_nthreads) default(shared) private(j) 
            {
                #pragma omp for
                for (j = 0;j < N;j++){
    ...
    #pragma omp barrier
    #pragma omp for private(i,j,resid) schedule(runtime) 
	    
	    //nthreads = omp_get_num_threads();//必须马上跟for循环，中间不能出现其他代码
        for(i = 0; i < M; i++){
            #pragma omp parallel num_threads(sub_nthreads) default(shared) private(j,resid) 
            {
...
//solve
    ...
    while (k < max_iter) {
        ...
        {
            omp_set_nested(1);
            error = Jacobi(f_1d, u_old, u_new, dx, dy,error,sub_nthreads);
        }
    ...
    printf("Finish at %d,use time:%.2f,use threads:%d,sub_nthreads:%d, the resid:%.2e,the err:%.2e\n",k,time1,nthreads,sub_nthreads,error,L1_err);
