...
int main(int argc,char *argv[]){
    ...
    while (k < max_iter) {
        ...
        if (size > 1){
            if (rank == 0){
                ...
                MPI_Send(&u_1d[(rank + 1)*step_y - 1][0],count,MPI_FLOAT,rank + 1,0,MPI_COMM_WORLD);
		        MPI_Recv(&u_1d[(rank + 1)*step_y][0],count,MPI_FLOAT,rank + 1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                
            }
            else if (rank > 0 && rank < size - 1) {
                count = step_x;
                tag = 0;
                MPI_Recv(&u_1d[rank*step_y - 1][0],count,MPI_FLOAT,rank - 1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                MPI_Send(&u_1d[rank*step_y][0],count,MPI_FLOAT,rank - 1,tag,MPI_COMM_WORLD);

                MPI_Send(&u_1d[(rank + 1)*step_y - 1][0],count,MPI_FLOAT,rank + 1,tag,MPI_COMM_WORLD);
		        MPI_Recv(&u_1d[(rank + 1)*step_y ][0],count,MPI_FLOAT,rank + 1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                ...
                MPI_Recv(&u_1d[rank*step_y - 1][0],count,MPI_FLOAT,rank - 1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                MPI_Send(&u_1d[rank*step_y][0],count,MPI_FLOAT,rank - 1,0,MPI_COMM_WORLD);
    ...
    if (rank == 0){
        printf("the process:%d,the max_iter:%d,Finish at %d,use time:%.2f, the resid:%.2e,the err:%.2e\n",size,max_iter,k,ela,rho,error);
    }
