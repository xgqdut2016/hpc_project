...
int main(int argc,char *argv[]){
    ...
    int lef = rank - 1,rig = rank + 1; // xu ni rank
    if (lef < 0){
        lef = MPI_PROC_NULL;
    }
    if (rig == size){
    ...
    int ind,ib[2] = {rank*step_x,(rank + 1)*step_x - 1};// rank boundary
    ...
    while (k < max_iter) {
        ...
        if (size > 1){
            ...
            {
                for(i = 0; i < N; i++){
                    MPI_Send(&u_1d[i][(rank + 1)*step_x - 1],count,MPI_FLOAT,rig,tag,MPI_COMM_WORLD);
		            MPI_Recv(&u_1d[i][(rank + 1)*step_x],count,MPI_FLOAT,rig,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);// yue jie warning
                }
            ...
            if (lef >= 0){
                for(i = 0; i < N; i++){
                    MPI_Recv(&u_1d[i][rank*step_x - 1],count,MPI_FLOAT,lef,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);// yue jie warning
                    MPI_Send(&u_1d[i][rank*step_x],count,MPI_FLOAT,lef,tag,MPI_COMM_WORLD);
                }
                
            }
            
            for(j = rank*step_x + 1; j < (rank + 1)*step_x - 1; j++){ //rank inside
                for(i = 0; i < N; i++){
                    x = bound[0][0] + i*hx,y = bound[1][0] + j*hy;
                    if (i == 0 || i == N - 1){
                        continue;
