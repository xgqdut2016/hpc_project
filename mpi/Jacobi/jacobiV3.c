...
int main(int argc,char *argv[]){
    ...
    while (k < max_iter) {
        ...
        if (size > 1){
            ...
            if (down < size)
            {
                MPI_Isend(&u_1d[(rank + 1)*step_y - 1][0],count,MPI_FLOAT,down,tag,MPI_COMM_WORLD,&req1);
		        MPI_Irecv(&u_1d[(rank + 1)*step_y][0],count,MPI_FLOAT,down,tag,MPI_COMM_WORLD,&req2);// yue jie warning
            }
            
            if (up >= 0){
                MPI_Irecv(&u_1d[rank*step_y - 1][0],count,MPI_FLOAT,up,tag,MPI_COMM_WORLD,&req3);// yue jie warning
                MPI_Isend(&u_1d[rank*step_y][0],count,MPI_FLOAT,up,tag,MPI_COMM_WORLD,&req4);
            }
