...
int main(int argc,char *argv[]){
    ...
    while (k < max_iter) {
        ...
        if (rank < 4){
            count = step_x;
            tag = 0;
            MPI_Sendrecv(&u_1d[(rank_y + 1)*step_y - 1][(rank_x)*step_x],count,MPI_FLOAT,rank + 4, tag,\
            &u_1d[(rank_y + 1)*step_y][rank_x*step_x],count,MPI_FLOAT,rank + 4, tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            
        }
        else if (rank >= 4 && rank < 12){
            count = step_x;
            tag = 0;
            MPI_Sendrecv(&u_1d[rank_y*step_y][rank_x*step_x],count,MPI_FLOAT,rank - 4, tag,\
            &u_1d[rank_y*step_y - 1][rank_x*step_x],count,MPI_FLOAT,rank - 4,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        ...
        if (rank%4 == 0){
            ...
            tag = 0;
            for(i = (rank_y)*step_y; i < (rank_y+ 1)*step_y; i++){
                MPI_Sendrecv(&u_1d[i][(rank_x + 1)*step_x - 1],count,MPI_FLOAT,rank + 1,tag,\
                &u_1d[i][(rank_x + 1)*step_x],count,MPI_FLOAT,rank + 1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            ...
            tag = 0;
            for(i = (rank_y)*step_y; i < (rank_y + 1)*step_y; i++){
                MPI_Sendrecv(&u_1d[i][(rank_x)*step_x],count,MPI_FLOAT,rank - 1,tag,\
                &u_1d[i][(rank_x)*step_x - 1],count,MPI_FLOAT,rank - 1,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        ...
        if (rank_x != 3){
            if (rank < 11){
                count = 1;
                tag = 0;
                MPI_Sendrecv(&u_1d[(rank_y + 1)*step_y - 1][(rank_x + 1)*step_x - 1],count,MPI_FLOAT,rank + 5,tag,\
                &u_1d[(rank_y + 1)*step_y][(rank_x + 1)*step_x],count,MPI_FLOAT,rank + 5,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            ...
            if (rank > 3){
                count = 1;
                tag = 0;
                MPI_Sendrecv(&u_1d[(rank_y)*step_y][(rank_x + 1)*step_x - 1],count,MPI_FLOAT,rank - 3,tag,\
                &u_1d[(rank_y)*step_y - 1][(rank_x + 1)*step_x],count,MPI_FLOAT,rank - 3,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
        }
        if (rank_x != 0){
            if (rank > 3){
                count = 1;
                tag = 0;
                MPI_Sendrecv(&u_1d[(rank_y)*step_y][(rank_x)*step_x],count,MPI_FLOAT,rank - 5,tag,\
                &u_1d[(rank_y)*step_y - 1][(rank_x)*step_x - 1],count,MPI_FLOAT,rank - 5,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
            if (rank < 12){
                count = 1;
                tag = 0;
                MPI_Sendrecv(&u_1d[(rank_y + 1)*step_y - 1][(rank_x)*step_x],count,MPI_FLOAT,rank + 3,tag,\
                &u_1d[(rank_y + 1)*step_y][(rank_x)*step_x - 1],count,MPI_FLOAT,rank + 3,tag,MPI_COMM_WORLD,MPI_STATUS_IGNORE); 
            }
        }
        //--------------------
