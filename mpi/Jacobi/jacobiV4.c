...
int main(int argc,char *argv[]){
    ...
    int ind,ib[2] = {rank*step_y,(rank + 1)*step_y - 1};// rank boundary
    all_time0 = MPI_Wtime();//------------------------------Jacobi start
    MPI_Status sta[4];
    MPI_Request req[4];
    int count = step_x,tag0 = 0,tag1 = 1;
    
    MPI_Send_init(&u_1d[(rank + 1)*step_y - 1][0],count,MPI_FLOAT,down,tag0,MPI_COMM_WORLD,&req[0]);
	MPI_Recv_init(&u_1d[(rank + 1)*step_y][0],count,MPI_FLOAT,down,tag1,MPI_COMM_WORLD,&req[1]);
    
    
    MPI_Send_init(&u_1d[rank*step_y][0],count,MPI_FLOAT,up,tag1,MPI_COMM_WORLD,&req[2]);
    MPI_Recv_init(&u_1d[rank*step_y - 1][0],count,MPI_FLOAT,up,tag0,MPI_COMM_WORLD,&req[3]);
    ...
    while (k < max_iter) {
        ...
        if (size > 1){
            ...
            if (down < size){
                MPI_Startall(2,&req[0]);
            }
            if (up >= 0){
                MPI_Startall(2,&req[2]);
            }
            
            for(i = rank*step_y + 1; i < (rank + 1)*step_y - 1; i++){ //rank inside
                for(j = 0; j < N; j++){
                    x = bound[0][0] + i*hx,y = bound[1][0] + j*hy;
                    if (j == 0 || j == N - 1){
                        continue;
