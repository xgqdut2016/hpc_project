...
double Jacobi(double *f_1d, double *u_old, double *u_new, double r1, double r2, double r3, double r, int M, int N){
    int j,i;
    double error = 0;
    double resid = 0;
    for(j = 0; j < N; j++){
...
void solve(double *f_1d, double *u_old, double *u_new, double eps, double r1, double r2, double r3, double r, int M, int N){
    int k = 0;
    double error;
    
    while (k < max_iter){
        error = Jacobi(f_1d,u_old, u_new, r1, r2, r3, r, M, N);
        if (error < eps){
            break;
        }
        k += 1;
...
int main(int argc, char **argv){
    ...
    double st,ela;
    st = get_walltime();
    solve(f_1d, u_old, u_new, eps, r1, r2, r3, r, M, N);
    ela = get_walltime() - st;
    double err = 0;
    err = L1_err(u_new, u, M, N);
    printf("CPU: use time:%.2f,the L1_err:%.4e\n",ela,err);
    double tmp[M*N];
    for(int j = 0; j < N; j++){
        for(int i = 0; i < M; i++){
            tmp[j*M + i] = u_new[j*M + i];
        }
    }
    // re_init
    init_data(bound, u_new, u, f_1d, dx, dy, M, N);
    st = get_walltime();
    cuda_solve(f_1d, u_old, u_new, eps, r1, r2, r3, r, M, N);
    ela = get_walltime() - st;
    err = 0;
    err = L1_err(u_new, tmp, M, N);
    printf("GPU: use time:%.2f,the L1_err of serial and parallel:%.4e\n",ela,err);
