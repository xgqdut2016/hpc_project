                          ...
                          int indK, float *val)
{
    ...
    for (int ph = 1; ph < width; ph++)
    {
        ...
        for (int index = 0; index < Bd; index++)
        {
            ...
            addSpecial(&val[0], com_a[0].x, com_b[0]);          // index_q = 0, 0<= index_v <= 3
            addSpecial(&val[4], com_a[0].x, com_b[1]);          // index_q = 0, 4<= index_v <= 7
            addSpecial(&val[Bk], com_a[0].y, com_b[0]);         // index_q = 1, 0<= index_v <= 3
            addSpecial(&val[Bk + 4], com_a[0].y, com_b[1]);     // index_q = 1, 4<= index_v <= 7
            addSpecial(&val[2 * Bk], com_a[0].z, com_b[0]);     // index_q = 2, 0<= index_v <= 3
            addSpecial(&val[2 * Bk + 4], com_a[0].z, com_b[1]); // index_q = 2, 4<= index_v <= 7
            addSpecial(&val[3 * Bk], com_a[0].w, com_b[0]);     // index_q = 3, 0<= index_v <= 3
            addSpecial(&val[3 * Bk + 4], com_a[0].w, com_b[1]); // index_q = 3, 4<= index_v <= 7

            addSpecial(&val[4 * Bk], com_a[1].x, com_b[0]);     // index_q = 4, 0<= index_v <= 3
            addSpecial(&val[4 * Bk + 4], com_a[1].x, com_b[1]); // index_q = 4, 4<= index_v <= 7
            addSpecial(&val[5 * Bk], com_a[1].y, com_b[0]);     // index_q =5, 0<= index_v <= 3
            addSpecial(&val[5 * Bk + 4], com_a[1].y, com_b[1]); // index_q = 5, 4<= index_v <= 7
            addSpecial(&val[6 * Bk], com_a[1].z, com_b[0]);     // index_q = 6, 0<= index_v <= 3
            addSpecial(&val[6 * Bk + 4], com_a[1].z, com_b[1]); // index_q = 6, 4<= index_v <= 7
            addSpecial(&val[7 * Bk], com_a[1].w, com_b[0]);     // index_q =7, 0<= index_v <= 3
            addSpecial(&val[7 * Bk + 4], com_a[1].w, com_b[1]); // index_q = 7, 4<= index_v <= 7

            // addSpecial(&tmp[0], com_a[0].x, com_b[0]);          // index_q = 0, 0<= index_v <= 3
            // addSpecial(&tmp[4], com_a[0].x, com_b[1]);          // index_q = 0, 4<= index_v <= 7
            // addSpecial(&tmp[Bk], com_a[0].y, com_b[0]);         // index_q = 1, 0<= index_v <= 3
            // addSpecial(&tmp[Bk + 4], com_a[0].y, com_b[1]);     // index_q = 1, 4<= index_v <= 7
            // addSpecial(&tmp[2 * Bk], com_a[0].z, com_b[0]);     // index_q = 2, 0<= index_v <= 3
            // addSpecial(&tmp[2 * Bk + 4], com_a[0].z, com_b[1]); // index_q = 2, 4<= index_v <= 7
            // addSpecial(&tmp[3 * Bk], com_a[0].w, com_b[0]);     // index_q = 3, 0<= index_v <= 3
            // addSpecial(&tmp[3 * Bk + 4], com_a[0].w, com_b[1]); // index_q = 3, 4<= index_v <= 7

            // addSpecial(&tmp[4 * Bk], com_a[1].x, com_b[0]);     // index_q = 4, 0<= index_v <= 3
            // addSpecial(&tmp[4 * Bk + 4], com_a[1].x, com_b[1]); // index_q = 4, 4<= index_v <= 7
            // addSpecial(&tmp[5 * Bk], com_a[1].y, com_b[0]);     // index_q =5, 0<= index_v <= 3
            // addSpecial(&tmp[5 * Bk + 4], com_a[1].y, com_b[1]); // index_q = 5, 4<= index_v <= 7
            // addSpecial(&tmp[6 * Bk], com_a[1].z, com_b[0]);     // index_q = 6, 0<= index_v <= 3
            // addSpecial(&tmp[6 * Bk + 4], com_a[1].z, com_b[1]); // index_q = 6, 4<= index_v <= 7
            // addSpecial(&tmp[7 * Bk], com_a[1].w, com_b[0]);     // index_q =7, 0<= index_v <= 3
            // addSpecial(&tmp[7 * Bk + 4], com_a[1].w, com_b[1]); // index_q = 7, 4<= index_v <= 7
        }
    ...
    for (int index = 0; index < Bd; index++)
    {
        ...
        (float4 &)com_a[1] = (float4 &)shareQK[index * numQ + threadIdx.y * Rq +
                                               4 + (ph - 1) % 2 * numQ * Bd];
        (float4 &)com_b[0] = (float4 &)
            shareVK[index * numK + threadIdx.x * Bk + (ph - 1) % 2 * numK * Bd];
        (float4 &)com_b[1] = (float4 &)shareVK[index * numK + threadIdx.x * Bk +
                                               4 + (ph - 1) % 2 * numK * Bd];
        addSpecial(&val[0], com_a[0].x, com_b[0]);          // index_q = 0, 0<= index_v <= 3
        addSpecial(&val[4], com_a[0].x, com_b[1]);          // index_q = 0, 4<= index_v <= 7
        addSpecial(&val[Bk], com_a[0].y, com_b[0]);         // index_q = 1, 0<= index_v <= 3
