    ...
    int indK, float *val)
{
    ...
    for (int ph = 1; ph < width; ph++)
    {
        ...
        {
            (float4 &)com_a[0] =
                (float4 &)shareQK[(index + (ph - 1) * Bd) * numQ + threadIdx.y * Rq];
        ...
        shareVK[(4 * smem_b_n) * numK + smem_b_k + (ph % 2) * numK * Bd] = b[0].x;
        shareVK[(4 * smem_b_n + 1) * numK + smem_b_k + (ph % 2) * numK * Bd] = b[0].y;
        shareVK[(4 * smem_b_n + 2) * numK + smem_b_k + (ph % 2) * numK * Bd] = b[0].z;
        shareVK[(4 * smem_b_n + 3) * numK + smem_b_k + (ph % 2) * numK * Bd] = b[0].w;
...
__device__ void matmulSV(float *shareSP, const float *__restrict inputV,
                         ...
                         float *sumSV)
{
    for (int index_k = 0; index_k < Bk; index_k++)
    {
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            {
                shareSP[(threadIdx.y * Rq + index_q) * Bc + threadIdx.x] =
                    __expf(val[index_q * Bk + index_k] - newMax[index_q]);
            }
            else
            {

                shareSP[(threadIdx.y * Rq + index_q) * Bc + threadIdx.x] = 0.0f;
            }
        }
        __syncthreads();
            ...
            for (int index_q = 0; index_q < Rq; index_q++)
            {
                for (int index_v = 0; index_v < Rv; index_v++)
                {
                    sumSV[index_q * Rv + index_v] +=
                        shareSP[(threadIdx.y * Rq + index_q) * Bc + phc] *
                                 ...
                                 float *__restrict output)
{
    ...
    __shared__ float shareQK[numQ * 128]; //[Bc,Br] = [16, 16], numQ = 64, shareQK = [128, 64] = Q.T
    ...
    {
        (float4 &)a[0] = (float4 &)inputQ[(indQ + tx) * d + (id * 16 + 4 * ty)];
        for (int index = 0; index < 4; index++)
        {
            if (id * 16 + 4 * ty + index < d)
            {
                shareQK[(id * 16 + 4 * ty + index) * numQ + tx] = a[index];
            }
            else
            {
                shareQK[(id * 16 + 4 * ty + index) * numQ + tx] = 0.0f;
            }
        }
    }
    __shared__ float shareVK[Bc * numV]; //[16, 128]
    __shared__ float shareSP[numQ * Bc]; //[64, 16]
    __shared__ float block_max[numQ];    //[4]
    ...
    for (int j = 0; j < Tc; j++)
    {
        ...
        int indK = j * numK;
        float val[Rq * Bk];
        memset(val, 0.0f, sizeof(val));
        matmulRQK(inputK, shareQK, shareVK, N, d, width, indQ, indK,
                  val);
        for (int index_q = 0; index_q < Rq; index_q++)
        {
        ...
        matmulSV(shareSP, inputV, shareVK, N, d, j, indQ, indK, indV, val,
                 newMax, sumSV);
