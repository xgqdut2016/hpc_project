                                 ...
                                 float *__restrict output)
{
    ...
    float *Vds = sram + Bc * Br * 2 + Bc * Br * sEnd + 2 * Br * Bc; //[Bc,Bc]
    float newMax;
    float oldMax;
    float newSum;
    newMax = -__FLT_MAX__;
    ...
    for (int j = 0; j < Tc; j++)
    {
        ...
        for (int index = 0; index < d; index++)
        {
            if (index < rate * Bc)
            {
                sum_qk = std::fma(Qds[threadIdx.y * sEnd * Bc + index], inputK[indK * d + index], sum_qk);
            }
            else
            {
                sum_qk = std::fma(inputQ[indQ * d + index], inputK[indK * d + index], sum_qk);
            }
        }
        if (indQ < N && indK < N)
        {
        ...
        if (threadIdx.y < Bc) { // threadIdx.y的范围必须>=Bc
        ...
        {
            sumSV[threadIdx.y * Bc + threadIdx.x] = std::fma(__expf(sumQK[threadIdx.y * Bc + phc] - newMax), Vds[threadIdx.x * Bc + phc], sumSV[threadIdx.y * Bc + threadIdx.x]);
        }
