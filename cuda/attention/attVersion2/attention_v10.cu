...
__device__ void matmulSV(float *Sds, const float *__restrict inputV,
                         ...
                         float *sumSV)
{
    ...
    for (int index_q = 0; index_q < Rq; index_q++)
    {
        for (int index_k = 0; index_k < Bk; index_k++)
        {
            ...
            {
                Sds[(threadIdx.y * Rq + index_q) * numK + threadIdx.x * Bk +
                    index_k] =
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            for (int index_v = 0; index_v < Rv; index_v++)
            {
                sumSV[index_q * Rv + index_v] +=
                    Sds[(threadIdx.y * Rq + index_q) * numK + phc] *
...
__global__ void _attentionKernel_128(const float *__restrict inputQ,
                                     const float *__restrict inputK,
                                     const float *__restrict inputV, int N,
                                     int d, float *__restrict output)
{

    __shared__ float Qds[numQ * 128];
    __shared__ float KVds[numK * 128 * 2]; // 128 = numV
    __shared__ float Sds[numQ * numK];
    ...
    for (int index_q = 0; index_q < Rq; index_q++)
    {
        ...
        for (int id = 4 * threadIdx.x; id < d; id += 4 * blockDim.x)
        {
            (float4 &)Qds[(threadIdx.y * Rq + index_q) * d + id] =
                (float4 &)inputQ[(indQ + threadIdx.y * Rq + index_q) * d + id];
        }
    }
    ...
    for (int j = 0; j < Tc; j++)
    {
        ...
        for (int index_k = 0; index_k < Bk; index_k++)
        {
            for (int id = 4 * threadIdx.y; id < d; id += 4 * blockDim.y)
            {
                (float4 &)a[0] = (float4 &)
                    inputK[(indK + threadIdx.x * Bk + index_k) * d + id];
                for (int index = 0; index < 4; index++)
                {
                    KVds[(id + index) * numK + threadIdx.x * Bk + index_k] =
                        a[index];
                }
            }
        }
            ...
            for (int index_q = 0; index_q < Rq; index_q++)
            {
                for (int index_k = 0; index_k < Bk; index_k++)
                {
                    ...
                    val[index_q * Bk + index_k] +=
                        Qds[(threadIdx.y * Rq + index_q) * d + index] *
                        KVds[index * numK + threadIdx.x * Bk + index_k];
                }
            }
        }
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            if (threadIdx.x == 0)
            {
                Sds[threadIdx.y * Rq + index_q] = tmpReduceMax;
            }
            __syncthreads();
            ...
            if (threadIdx.x == 0)
            {
                Sds[threadIdx.y * Rq + index_q + numQ] = tmpReduceSum;
            }
            __syncthreads();
            if (newMax[index_q] > Sds[threadIdx.y * Rq + index_q])
            {
                newSum[index_q] = std::fma(
                    Sds[threadIdx.y * Rq + index_q + numQ],
                    __expf(Sds[threadIdx.y * Rq + index_q] - newMax[index_q]),
                    newSum[index_q]);
            }
            else
            {
                newSum[index_q] = std::fma(
                    newSum[index_q],
                    __expf(newMax[index_q] - Sds[threadIdx.y * Rq + index_q]),
                    Sds[threadIdx.y * Rq + index_q + numQ]);

                newMax[index_q] = Sds[threadIdx.y * Rq + index_q];
            }
            // PV
            for (int index_v = 0; index_v < Rv; index_v++)
            {
        ...
        matmulSV(Sds, inputV, KVds, N, d, j, indQ, indK, indV, val, newMax,
                 sumSV);
...
void attention(float *cpu_Q, float *cpu_K, float *cpu_V, int N, int d, float *cpu_output)
{
    ...
    _attentionKernel_128<Br, Bc, Rq, Rv>
