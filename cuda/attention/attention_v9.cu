                         ...
                         float *newMax, float *sumSV)
{
    ...
    {
        if (threadIdx.x + j * Bc < N && indV + index_v < d)
        {
            Vds[threadIdx.x * Rv + index_v] =
                inputV[(threadIdx.x + j * Bc) * d + indV + index_v];
        }
        else
        {
            Vds[threadIdx.x * Rv + index_v] = 0.0f;
        }
    }
    for (int index_q = 0; index_q < Rq; index_q++)
    {
    ...
    for (int index_q = 0; index_q < Rq; index_q++)
    {
        for (int index_v = 0; index_v < Rv; index_v++)
        {
            float val = sumQK[(threadIdx.y * Rq + index_q) * Bc + threadIdx.x] *
                        Vds[threadIdx.x * Rv + index_v];
            val = WarpAllReduce<SumOp, float, Bc>(val);
            if (threadIdx.x == 0)
            {
                sumSV[index_q * Rv + index_v] += val;
            }
            __syncthreads();
        }
    }
        ...
        for (int index_v = 0; index_v < Rv; index_v++)
        {
            if (indQ + index_q < N && indV + index_v < d)
            {
                output[(indQ + index_q) * d + indV + index_v] =
                    out[index_q * Rv + index_v] *
                    __fdividef(1.0F, newSum[index_q]);
            }
        }
    }
}
