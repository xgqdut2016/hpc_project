                          ...
                          int indK, float *val)
{
    ...
    {
        for (int w = 0; w < numK; w += Bd)
        {
            int Q_smem = (tid / 4) * numK + (tid % 4) * 4 + w;
            int Q_gmem = (indQ + tid / 4) * d + numK * ph + (tid % 4) * 4 + w;
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            {
                if (indQ + index_q + threadIdx.y * Rq >= N ||
                    (threadIdx.x + ph * Bc) * Bk + index_k >= d)
                {
                    shareQK[(threadIdx.y * Rq + index_q) * numK +
        ...
        for (int w = 0; w < numK; w += Bd)
        {
            int K_gmem = (indK + tid % 64) * d + numK * ph + (tid / 64) * 4 + w;
            (float4 &)a[0] = (float4 &)inputK[K_gmem];
            for (int id = 0; id < 4; id++)
            {
                shareVK[((tid / 64) * 4 + w + id) * numK + (tid % 64)] = a[id];
            }
        }
        for (int w = 0; w < numK; w += Bd)
        {
            ...
            {
                if (indK + tid % 64 >= N ||
                    numK * ph + (tid / 64) * 4 + w + id >= d)
                {
                    shareVK[((tid / 64) * 4 + w + id) * numK + (tid % 64)] =
                        0.0f;
                }
            }
        }
        ...
        //     for (int index_k = 0; index_k < Bk; index_k++) {
        //         if (indK + index_k + threadIdx.x * Bk >= N ||
        //             (threadIdx.y + ph * Bc) * Bk + idk >= d) {
        //             shareVK[(threadIdx.y * Bk + idk) * numK + threadIdx.x *
        //             Bk +
        //                     index_k] = 0.0f;
        //         }
        //     }
        // }
        __syncthreads();
        for (int index = 0; index < numK; index++)
        {
            for (int index_q = 0; index_q < Rq; index_q++)
            {
                for (int index_k = 0; index_k < Bk; index_k++)
                {
                    ...
                    val[index_q * Rq + index_k] +=
                        shareQK[comp_a_smem_m * numK + index] *
