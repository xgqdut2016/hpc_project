    ...
    for (int j = 0; j < Tc; j++)
    {
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            for (int ph = 0; ph < width; ph++)
            {
                ...
                {
                    val = std::fma(Qds[(threadIdx.y * Rq + index_q) * Bc + index],
                                   Kds[index * Bc + threadIdx.x], val);
                }
                __syncthreads();
            }
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            ...
            {
                regTmp[index_q] =
                    __expf(regQK[index_q] - block_max[index_q][threadIdx.y]);
            }
            else
            {
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            if (indQ + index_q < N && indK < N)
            {
                sumQK[(threadIdx.y * Rq + index_q) * Bc + threadIdx.x] =
                    __expf(regQK[index_q] - newMax[index_q]);
            }
            else
            {
