                         ...
                         float *val, float *newMax, float *sumSV)
{
    ...
    if (threadIdx.y < Bc)
    {
        for (int idv = 0; idv < Rv; idv++)
        {
            for (int index_v = 0; index_v < Rv; index_v++)
            {
                if ((threadIdx.y + j * Bc) * Rv + idv < N &&
                    indV + index_v < d)
                {
                    shareVK[(threadIdx.y * Rv + idv) * Bc * Rv +
                            threadIdx.x * Rv + index_v] =
                        inputV[((threadIdx.y + j * Bc) * Rv + idv) * d + indV +
                               index_v];
                }
                else
                {
    ...
    __syncthreads();
    for (int phc = 0; phc < Bc * Rv; phc++)
    {
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            regLeft[index_q] =
                shareQK[(threadIdx.y * Rq + index_q) * Bc * Rv + phc];
        }
        //--------
        for (int index_v = 0; index_v < Rv; index_v++)
        {
                                 ...
                                 float *__restrict output)
{
    ...
    for (int j = 0; j < Tc; j++)
    {
        ...
        for (int index_q = 0; index_q < Rq; index_q++)
        {
            for (int index_v = 0; index_v < Rv; index_v++)
            {
                sumSV[index_q * Rv + index_v] *= __expf(oldMax[index_q] - newMax[index_q]);
            }
        }
