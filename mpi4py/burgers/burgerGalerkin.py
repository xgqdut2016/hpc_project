...
class INSET():
    def __init__(self,prob,bound,size_tr,size,rank,dtype,dev,para):
        ...
        self.ff = torch.zeros(self.Node.shape[0],4,1)
        self.u_acc = torch.zeros(self.Node.shape[0],4,1)
        for i in range(self.Node.shape[0]):
            self.v[i,:,0:1] = self.basic(self.X[i,:,:],[0,0],i).view(-1,1)
            self.ff[i,:,0:1] = FF(self.X[i,:,:],prob).view(-1,1)
            self.u_acc[i,:,0:1] = UU(self.X[i,:,:],[0,0],prob).view(-1,1)
        ...
        self.v = self.v.data
        self.vx = self.vx.data
        self.ff = self.ff.data
        self.u_acc = self.u_acc.data
...
def Loss_fun(netu,inset,bdset):
    ...
    out_in = (ux*inset.vx).sum(2) - (inset.ff*inset.v).sum(2)
    inset.res_u = (out_in.mean(1))**2
