...
class INSET():
    def __init__(self,bound,size_tr,prob,a,b,dtype,dev):
        self.dim = 2
        ...
        self.u_acc = UU(self.x,[0,0],prob,a,b).reshape(-1,1).type(dtype)
        self.ff = FF(self.x,prob,a,b).reshape(-1,1).type(dtype)
    ...
    def quasi_samples(self,N):
        ...
        return sample
class BDSET():#边界点取值
    def __init__(self,bound,nx,prob,a,b,dtype,dev):
        self.dim = 2
        self.DS = 2*(nx[0] + nx[1])
        ...
        for j in range(nx[1]):
            ...
            self.x[m,1] = bound[1,0] + (j + 0.5)*self.hx[1]
            m = m + 1
        self.Dright = UU(self.x,[0,0],prob,a,b).reshape(-1,1)
...
N = 10
mu = np.zeros([M*N,2])
hx = [(abbound[0,1] - abbound[0,0])/(M -1),(abbound[1,1] - abbound[1,0])/(N - 1)]
for j in range(N):
    for i in range(M):
        mu[j*M + i,0] = abbound[0,0] + i*hx[0]
        mu[j*M + i,1] = abbound[1,0] + j*hx[1]
...
a = mu[j*M + i,0]
b = mu[j*M + i,1]
inset = INSET(bound,size_tr,prob,a,b,dtype,device)
bdset = BDSET(bound,nx_bd,prob,a,b,dtype,device)
